...
public class ArticleSocialDialog extends DialogFragment implements
	...
	public static ArticleSocialDialog newInstance(long articleId, String title,
			String imgUrl, String from) {
	...
	public static ArticleSocialDialog newInstance(String image, String title,
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (mArticleId > 0) {
			mSharedContent = getArguments().getString(KEY_ARTICLE_TITLE);
			...
			try {
				...
				mSinaSharedUMImage = new UMImage(getActivity(), bitmapArray);
				mSharedUMImage = mSinaSharedUMImage;
			} catch (Exception e) {
				dismiss();
				return;
			}
			mSharedContent = getArguments().getString(KEY_SHARED_TITLE);
	...
	private void social(int tag) {
		...
		case R.string.article_social_qq:
			...
			StatsUtil.statsReportByMta(getActivity(),"stats_share", "share_type",
					"qq", KEY_SHARED_FROM, mSharedFrom);
