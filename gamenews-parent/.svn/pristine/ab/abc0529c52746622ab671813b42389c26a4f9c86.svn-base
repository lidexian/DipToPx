...
public class ChannelSearchActivity extends BaseActivity implements
	...
	private ChannelGridFragment mChannelGridFragment;
	...
	public void onCreate(Bundle savedInstanceState) {
			...
			public void onClick(View v) {
				...
				StatsUtil.statsReport(ChannelSearchActivity.this,
						"stats_search_channel", "serarch_key", mSearch
								.getText().toString());
				StatsUtil.statsReportByMta(ChannelSearchActivity.this,
						"stats_search_channel", "serarch_key", mSearch
						.getText().toString());
		...
		if (savedInstanceState != null) {
			mChannelGridFragment = (ChannelGridFragment) getSupportFragmentManager()
	...
	public void setSearchResultsView(String key, String attachInfo,
		...
		if (mChannelGridFragment == null) {
			mChannelGridFragment = ChannelGridFragment.newInstance(key,
					attachInfo, hasMore, channels);
		}
		if (!isDestroyed()) {
			FragmentTransaction transaction = getSupportFragmentManager()
					.beginTransaction();
			if (!mChannelGridFragment.isAdded()) {
				transaction.add(R.id.fragment_container, mChannelGridFragment,
						FG_TAG_CHANNEL_GRID);
			} else {
				mChannelGridFragment.refreshSearchResults(key, hasMore,
						attachInfo, channels);
			}
			...
			transaction.show(mChannelGridFragment);
	...
	public void setSearchNoResultsView(ArrayList<Column> columns) {
		...
		if (!isDestroyed()) {
			...
			if (mChannelGridFragment != null && mChannelGridFragment.isAdded()
					&& !mChannelGridFragment.isHidden()) {
				transaction.hide(mChannelGridFragment);
			}
