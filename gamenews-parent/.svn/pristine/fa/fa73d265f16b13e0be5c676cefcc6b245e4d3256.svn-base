...
public class ArticleDetailActivity extends BaseActivity implements
	...
	private List<String> mSrcImages;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (ARTICLE_UNCERTAIN != mArticleInfo.getArticleType()) {
			StatsUtil.statsReport(this, "stats_read_article", "article_name",
					mArticleInfo.getTitle());
			StatsUtil.statsReportByMta(this, "stats_read_article",
					"article_name", mArticleInfo.getTitle());
		...
		if (!afterCache) {
			...
			if (Constants.isFunctionEnabled(Constants.APPTYPE_GAMENEWS)) {
				filepath = dir.getAbsolutePath() + "/" + Constants.NEWS_HTML;
					...
					.isFunctionEnabled(Constants.APPTYPE_SPORTBRUSH)) {
	...
	class JsInterface {
		...
		public void openImage(String img) {
			...
			intent.putExtra(ImageDetailViewerActivity.EXTRA_CURRENT_IMAGE, 0);
			startActivity(intent);
		}
		...
		public void onClickImage(int id) {
			Intent intent = new Intent(ArticleDetailActivity.this,
					ImageDetailViewerActivity.class);
			intent.putExtra(ImageDetailViewerActivity.EXTRA_IMAGE_LIST,
					(Serializable) mArticleDetail.getImageList());
			intent.putExtra(ImageDetailViewerActivity.EXTRA_CURRENT_IMAGE, id);
			startActivity(intent);
	...
	public void onClick(View v) {
		...
		case R.id.report_btn:
			...
			showDialog(fr, TAG_REPORT_DIALOG);
			break;
		case R.id.keep_btn:
			if (mArticleDetail == null) {
				return;
